    ...
    def validate_and_save(self):
        ...
        if self.selected_choice == "Yes":
            if not os.path.isfile(path):
                ...
                return
            self.app.state["categories"] = load_categories(path)
        else:
    ...
    def add_expense(self):
        ...
        if dlg.result:
            ...
            for i, (ename, eqty, ecost) in enumerate(self.expenses):
                if ename == name:
                    self.expenses[i] = (ename, eqty + qty, cost)  # update quantity
                    self.update_content()
                    return
...
class CategoryScreen(ScreenBase):
    ...
    def on_show(self):
        ...
        for cname, items in cats.items():
            expenses = [(iname, qty, cost) for iname, (qty, cost) in items.items()]

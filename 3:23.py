    ...
    def validate_and_save(self):
        ...
        if self.selected_choice == "Yes":
            if not os.path.isfile(path):
                ...
                return
            # Load categories, but only keys for GUI, ignore past expenses
            loaded = load_categories(path)
            self.app.state["categories"] = {k:{} for k in loaded.keys()}
        else:
...
class CategoryBox(tk.Frame):
    ...
    def __init__(self, master, category_name, remove_callback, expenses=None):
    ...
    def add_expense(self):
        ...
        if dlg.result:
            ...
            for i, (ename, eqty, ecost) in enumerate(self.expenses):
                if ename == name:
                    self.expenses[i] = (ename, eqty + qty, cost)
                    self.update_content()
                    return
...
class CategoryScreen(ScreenBase):
    ...
    def on_show(self):
        ...
        for cname in cats.keys():
            # Load with empty expenses
            box = CategoryBox(self.box_frame, cname, self.remove_category, expenses=[])
    ...
    def finish(self):
        ...
        for box in self.category_boxes:
            if box.expenses:  # Only save categories with new expenses
...
class SummaryScreen(ScreenBase):
    ...
    def on_show(self):
        ...
        if category_totals:
            ...
            # Create pie chart (hide labels on slices for cleaner look)
...
# -------------------------------------------------------------
# HELPER: LOAD CATEGORIES FROM FILE
# -------------------------------------------------------------
def load_categories(path):
    categories = {}
    if not os.path.isfile(path): return categories
    with open(path, "r", encoding="utf-8") as f:
        ...
        for line in f:
            ...
            if line.endswith(":"):
                ...
                m = re.match(r"(.+) \(x(\d+)\) : \$([0-9.]+)", line)
                if m and current_cat is not None:
                    name, qty, cost = m.groups()
                    categories[current_cat][name] = (int(qty), float(cost))
    return categories
